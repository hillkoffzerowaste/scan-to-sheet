...
export async function hillkoffRequest({ path, method = 'GET', query, body, role = '', fetchImpl = fetch, apiKey = process.env.HILLKOFF_API_KEY, timeoutMs = 15_000 }) {
  ...
  try { response = await fetchImpl(url.toString(), { method, headers: { 'x-api-key': apiKey, 'x-request-id': requestId, ...(role ? { 'x-api-role': role } : {}), ...(body ? { 'Content-Type': 'application/json' } : {}) }, body: body ? JSON.stringify(body) : undefined, cache: 'no-store', signal: AbortSignal.timeout(timeoutMs) }); }
