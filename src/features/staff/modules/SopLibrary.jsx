...
function nextReviewDate(effectiveDate) {
  const [year, month, day] = effectiveDate.split("-").map(Number);
  return new Date(Date.UTC(year + 1, month - 1, day)).toISOString().slice(0, 10);
}

function newSop(staff) {
  const effectiveDate = bangkokDateKey();
  return {
    title: "",
    description: "",
    ownerStaffId: staff.find((person) => person.active !== false)?.id ?? "",
    effectiveDate,
    reviewDueDate: nextReviewDate(effectiveDate),
    latestVersion: 0,
    active: true,
    steps: [createStep()],
  };
...
function editSop(sop, staff) {
  const effectiveDate = sop.effectiveDate || bangkokDateKey();
  return {
    id: sop.id,
    title: sop.title ?? "",
    description: sop.description ?? "",
    ownerStaffId: sop.ownerStaffId || staff.find((person) => person.active !== false)?.id || "",
    effectiveDate,
    reviewDueDate: sop.reviewDueDate || nextReviewDate(effectiveDate),
...
export default function SopLibrary({ sops, staff = [], isAdmin, firebaseUser, onRefresh, onMessage, onError }) {
  ...
  async function save(publish) {
    ...
    if (errors.length) {
      onError?.("กรุณาระบุชื่อ เจ้าของ SOP วันที่เริ่มใช้ รอบทบทวน และขั้นตอนให้ครบก่อนบันทึก");
      return;
    }
    setSaving(true);
    try {
      ...
      else if (error?.code === "STAFF_SOP_INVALID") onError?.("กรุณาตรวจชื่อ SOP เจ้าของ วันที่เริ่มใช้ รอบทบทวน และชื่อขั้นตอนให้ครบ");
  ...
  return (
    <div className="staff-sop-module">
      <div className="staff-module-header">
        ...
        {isAdmin && <button className="primary-action" type="button" disabled={sops.length >= WORK_PLANNING_QUERY_LIMITS.sops || !activeStaff.length} onClick={() => setEditor(newSop(staff))}><Plus size={16} /> สร้าง SOP</button>}
      </div>
