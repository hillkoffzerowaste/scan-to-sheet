...
- **กระดานรีโมทแยกใบตามโหมด** (`scanRemoteControl/packer` และ `scanRemoteControl/drive`) เพราะโหมด Packer กับ Drive ใช้ `selectedCourier` ตัวเดียวกัน ถ้ารวมกระดานกัน ห้องแพ็คจะเปลี่ยนขนส่งให้เครื่อง Admin ที่กำลังรับเข้า Drive อยู่โดยไม่มีใครรู้ และพัสดุชิ้นถัดไปจะลงขนส่งผิด — effect ทั้งสองใน `src/App.jsx` จึงผูกกับ `activeTab` และไม่ทำงานเลยในแท็บรายงาน/พนักงาน
- **effect ที่เขียนค่ากลับต้องข้ามรันแรกของแต่ละแท็บ** (`remoteControlTabRef`) เพราะการสลับแท็บไม่ใช่การเลือกของคน ค่าที่ค้างมาจากแท็บก่อนจะไปทับค่าที่รีโมทตั้งไว้
